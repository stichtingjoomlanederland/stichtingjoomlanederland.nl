...
class XmlSerializer
{
	...
	public function serialize(AbstractRequest $input)
	{
		...
		{
			if ($className === "iDEALConnector\Entities\AcquirerTransactionRequest")
			{
			...
			{
				if ($className === "iDEALConnector\Entities\AcquirerStatusRequest")
				{
	...
	{
		$element->appendChild(new DOMElement("createDateTimestamp",
			$request->getCreateDateTimestamp()->format("Y-m-d\TH:i:s\Z")));
	...
	private function deserializeAcquirerStatusResponse(DOMElement $xml, DateTime $createdTimestamp)
	{
		...
		$timestamp     = new DateTime($this->getFirstValue($node, "statusDateTimestamp",
